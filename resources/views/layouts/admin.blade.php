        ...
        <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-5">
            <div class="flex flex-wrap items-center gap-8">
                ...
                <nav class="flex items-center gap-5 text-sm text-white/75" aria-label="Admin navigation">
                    <a href="{{ route('admin.dashboard') }}" @class(['text-white' => request()->routeIs('admin.dashboard')])>Dashboard</a>
                    @if (auth()->user()->canAccessAdminArea('products'))
                        <a href="{{ route('admin.products.index') }}" @class(['text-white' => request()->routeIs('admin.products.*')])>Products</a>
                    @endif
                    @if (auth()->user()->canAccessAdminArea('orders'))
                        <a href="{{ route('admin.orders.index') }}" @class(['text-white' => request()->routeIs('admin.orders.*')])>Orders</a>
                    @endif
                    @if (auth()->user()->canAccessAdminArea('analytics'))
                        <a href="{{ route('admin.analytics') }}" @class(['text-white' => request()->routeIs('admin.analytics')])>Analytics</a>
                    @endif
                    @if (auth()->user()->role === 'super_admin')
                        <a href="{{ route('admin.temporary-access.index') }}" @class(['text-white' => request()->routeIs('admin.temporary-access.*')])>Temporary access</a>
                        <a href="{{ route('admin.account.edit') }}" @class(['text-white' => request()->routeIs('admin.account.*')])>Account</a>
                    @endif
                </nav>
            </div>
            <div class="flex items-center gap-5">
                ...
                <form method="post" action="{{ route('admin.logout') }}">
                    @csrf
                    <button class="text-sm" type="submit">Sign out</button>
                </form>
            </div>
        </div>
    </header>
